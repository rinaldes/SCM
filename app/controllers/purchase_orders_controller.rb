class PurchaseOrdersController < ApplicationController
    ...
    if params[:commit] == 'Upload Data Product'
      @purchase_order = PurchaseOrder.new(
        purchase_order_params.merge(
          date: Time.now,
          top: Supplier.find_by_id(params[:purchase_order][:supplier_id]).long_term || 30,
          status: PurchaseOrder::WAITING_APPROVAL,
      ...
      @purchase_order = PurchaseOrder.new(
        purchase_order_params.merge(
          date: Time.now, total_qty: values_details.map{|po|po[:quantity].to_i}.sum,
          number: "PO#{Time.now.year}#{sprintf('%06d', (PurchaseOrder.last.id rescue 0)+1)}",
  ...
  def autocomplete_purchase_order
    hash = []
    conditions = []
    conditions << params[:custom_condition] if params[:custom_condition].present?
    ...
    @po = PurchaseOrder.includes(:supplier).where(conditions.join(' AND ')).limit(200).order('number')
    @po.collect do |p|
      supplier = p.supplier
      hash << {
  ...
  def update
    ...
    if @purchase_order.update_attributes(
      purchase_order_params.merge(total_qty: values_details.map{|po|po[:quantity].to_i}.sum,
                grand_total: values_details.map{|po|po[:hpp].to_i*po[:quantity].to_i*po[:fraction].to_i}.sum)
      )
  ...
  private
    def purchase_order_params
      params.require(:purchase_order).permit(
        ...
        :head_office_id, :top, :status, :purchase_request_id,
    ...
    def get_paginated_purchase_orders
      ...
      @purchase_orders = PurchaseOrder
        .select("purchase_orders.id, purchase_orders.head_office_id, number, date, suppliers.name AS supplier_name, status, grand_total, total_qty, note, expected_delivery_date, due_date, office_name")
        .where(conditions.join(' AND ')).joins(:supplier, :office).order(default_order('purchase_orders')).paginate(page: params[:page], per_page: PER_PAGE) || []
    end

class ProductTransferReceiptsController < ApplicationController
#  before_filter :can_view_transfer_receipt, only: [:index, :show]
 # before_filter :can_manage_mutation_receipt, only: [:accept, :mark_as_received]
  ...
  def index
    ...
    @product_receipts = PurchaseTransfer.where(conditions.join(' AND ')).order(default_order('product_mutations'))
    .joins(
  ...
  def show
    ...
    render file: "purchase_transfers/show"
  end
  ...
  def mark_as_received
    ...
    redirect_to product_transfer_receipt_url(@product_receipt)
  end
